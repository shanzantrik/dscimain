...
@section('title', 'Edit Agenda - ' . $eventDay->title)
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Edit Agenda for {{ $eventDay->title }}</h3>
        ...
        <div class="card-body">
          <form action="{{ route('admin.event-days.agendas.update', [$eventDay, $agenda]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input type="time" name="start_time" id="start_time"
                    ...
                    value="{{ old('start_time', $agenda->start_time->format('H:i')) }}" required>
                  @error('start_time')
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input type="time" name="end_time" id="end_time"
                    ...
                    value="{{ old('end_time', $agenda->end_time->format('H:i')) }}" required>
                  @error('end_time')
                  <span class="invalid-feedback">{{ $message }}</span>
                  @enderror
                </div>
            ...
            <div class="form-group">
              ...
              <textarea name="description" id="description" rows="6"
                class="form-control tinymce @error('description') is-invalid @enderror">{{ old('description', $agenda->description) }}</textarea>
              @error('description')
              <span class="invalid-feedback">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div class="form-group">
              ...
              <div class="speakers-container">
                @forelse($agenda->speakers as $agendaSpeaker)
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <select name="speakers[]" class="form-control">
                        ...
                        <option value="{{ $speaker->id }}" {{ $agendaSpeaker->id == $speaker->id ? 'selected' : '' }}>
                          {{ $speaker->name }}
                        </option>
                        @endforeach
                      </select>
                    ...
                    <div class="col-md-5">
                      <input type="text" name="speaker_roles[]" class="form-control"
                        placeholder="Role (e.g., Moderator, Speaker)" value="{{ $agendaSpeaker->pivot->role }}">
                    </div>
                    <div class="col-md-1">
                      <button type="button" class="btn btn-danger btn-sm remove-speaker">
